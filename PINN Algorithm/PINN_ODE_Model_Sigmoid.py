...
class ODEPINN:
    def __init__(
        ...
        Trainable:
            - NN weights/biases
            - ODE parameters: p2, p3, p5, p7, p9 (clipped positive)
    ...
    def total_loss(self, t_physics, t_fit, w_fit, x_fit, y_fit, z_fit, t0, W0):
        ...
        r1 = dw - ((p1 + p3 * (x + y) / (self.x0_tf + self.x0_tf))* w * exp_term - self.p4 * w)
        r2 = dW - w
        ...
        for ep in range(int(epochs)):
            ...
            if (verbose_every is not None) and (verbose_every > 0) and (ep % int(verbose_every) == 0):
                ...
                print(
                    ...
                    f"p1={self.p1_value().numpy():.2e} p2={self.p2.numpy():.2e} p3={self.p3_value().numpy():.2e} "
...
def run_all_sheets(
    ...
    summary_csv = os.path.join(output_dir, "summary_pinn_sigmoid.csv")
...
if __name__ == "__main__":
    run_all_sheets(
        ...
        output_dir="results_PINN_sigmoid",    # Folder for saving results
