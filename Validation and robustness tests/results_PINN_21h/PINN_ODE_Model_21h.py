...
class ODEPINN:
    def __init__(
        ...
        Trainable:
            - NN weights/biases
            - ODE parameters: p1, p2, p3, p5, p7, p9 (clipped positive)
        ...
        self.p1 = tf.Variable(0.5, dtype=tf.float32, constraint=clip, name="p1")
        self.p2 = tf.Variable(init_val, dtype=tf.float32, constraint=clip, name="p2")
        self.p3 = tf.Variable(0.5, dtype=tf.float32, constraint=clip, name="p3")
    ...
    def total_loss(self, t_physics, t_fit, w_fit, x_fit, y_fit, z_fit, t0, W0):
        ...
        r1 = dw - ((self.p1 + self.p3 * (x + y) / (self.x0_tf + self.y0_tf))* w * exp_term - self.p4 * w)
        r2 = dW - w
    ...
    def train_step(self, t_physics, t_fit, w_fit, x_fit, y_fit, z_fit, t0, W0):
        ...
        param_vars = [self.p1, self.p2, self.p3, self.p5, self.p7, self.p9]
        ic_vars = [self.w0_var]
        ...
        for ep in range(int(epochs)):
            ...
            if (verbose_every is not None) and (verbose_every > 0) and (ep % int(verbose_every) == 0):
                ...
                print(
                    ...
                    f"p1={self.p1.numpy():.2e} p2={self.p2.numpy():.2e} p3={self.p3.numpy():.2e} "
                    f"p5={self.p5.numpy():.2e} p7={self.p7.numpy():.2e} p9={self.p9.numpy():.2e} "
                    f"| w0={self.w0_var.numpy():.2e}"
                )
...
def run_all_sheets(
    ...
    for sheet in sheet_names:
        ...
        preprocess_series_csv_path = os.path.join(output_dir, f"{sheet_str}_preprocess_series_21h.csv")
        ...
        pred_csv_path = os.path.join(output_dir, f"{sheet_str}_PINN_prediction_21h.csv")
        ...
        pdf_path = os.path.join(output_dir, f"{sheet_str}_PINN_report_21h.pdf")
        csv_table_path = os.path.join(output_dir, f"{sheet_str}_stats_table_21h.csv")
    ...
    summary_csv = os.path.join(output_dir, "summary_pinn_21h.csv")
...
if __name__ == "__main__":
    run_all_sheets(
        excel_path="data_matlab_smooth_21h.xlsx", # Data smoothed previously in MATLAB for consistency
        output_dir="results_PINN_21h", # Folder for saving results
        ...
        verbose_every=10000,             # Preliminary results are shown every number of epochs
